...
> 评测规模：1000 seed tasks + 400 benchmark tasks（50 基础 × 8 参数变体），2026-07-31 run_eval.py 实测。

| 指标 | 值 | 说明 |
|------|-----|------|
| 分类器 Macro F1 | 1.00 | 5 分类，571 训练 / 246 测试，合成数据下每类全部分对 |
| 失败率下降 | -60%（相对） | 20% baseline → 8% optimized，三档扩大验证收敛于 59-61% |
| Token 消耗下降 | -44.6% | KDE 默认值省略参数 + 规则前置校验减少重试 |
| 重试次数下降 | -60.0% | 规则前置校验在调用前拦截参数错误 |
| DAG 模式召回 | 100.0% | 3 预埋模式全找回（WL 图哈希升级后），额外发现 5 个高频子模式 |
| 参数模板覆盖率 | 71.4%（5/7） | 5 种工具有足够样本生成 KDE 模板，2 种缺 benchmark 任务使用 |
| 规则准确率 | 100% | 5 条规则全部有效，按错误类型硬编码映射 |
| 吞吐量 | 57 traces/s | 批量写入 |
| 治理动作 | 2 晋升 + 1 回滚 + 0 离线 | 模拟 60 次调用，A/B 回滚当前使用硬编码参数触发 |
| 测试覆盖 | 88 tests | 全部通过 |
