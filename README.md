...
- **参数分布学习**：对历史成功调用的参数做核密度估计（scipy gaussian_kde），自动给出默认值和合法边界（2.5%/97.5% 百分位），替代人工经验配置。支持用户偏好反向注入——MCP 记忆里存的偏好值优先于统计默认值。注：KDE 的价值在分布形状与 CI 边界（2000 规模实测边界外总比例 0.86%、per-param 最大 6.5%——双分母口径），点估计（mode）在 MAE 口径上不优于中位数——不做"默认值更准"声明
...
**离线批处理架构，不做实时拦截**。TEE 设计为离线分析管道——轨迹采集后批量处理，不是 Agent 每次工具调用前的同步拦截器。原因：KDE 参数学习、DAG 子图枚举、信用评分更新都是统计计算，需要足够样本量才有意义（`min_samples=30`）。对实时性要求高的场景（如每次工具调用前必须校验参数），应该在 Agent 代码中直接嵌入 `CounterfactualDistiller` 产出的规则——这些规则是纯确定性逻辑，可以同步执行。修复建议同样离线生成（蒸馏时批量调 LLM），触发时直接查表返回。全管道 run_eval 耗时 171.1s（2000 seed + 2000 benchmark 新口径，2026-08-27 实测）；执行层 2000 任务评测 ~55min（2000 次 LLM 规划，离线批量统计非产品延迟），适合小时级/日级调度，不适合毫秒级在线决策。
...
> 评测规模（新口径，2026-08-27 终态 E 全维度重跑）：**2000 seed tasks + 2000 benchmark tasks（50 基础 × 40 参数变体）+ 三档 degradation（500/1000/2000）+ 执行层 2000 任务对比评测**（live 实测），run_eval.py + run_execution_eval.py 实测（种子固化 _MAIN_RNG=42；2000 规模二次复跑确定性字段逐字段一致，唯一差异=LLM 输出长度自然方差）。**口径切换声明**：旧口径（1000 seed/400 benchmark/50 任务）已封存于 metrics-snapshot 旧口径段——新旧口径不可比，禁止混用。指标来源：template-data/metrics-snapshot.md（新口径段，2026-08-27）。

| 指标 | 值 | 说明 |
|------|-----|------|
| 分类器 Macro F1 | 1.000 | 5 分类，1155 训练 / 496 测试，504 维特征（含 has_cjk）；**纯种子口径**（阶段 10 隔离后只吃 eval-* 种子，旧 0.982 为混简化场景残留口径） |
| 失败率下降 | **-61.4%**（率口径）/ **-65.7%**（次数口径） | 失败率 0.1689→0.0652；1195→410 failures，2000 benchmark，注入全部 5 种错误类型 |
| Token 消耗下降 | **-45.2%** | 1,134,375→621,750 tokens，KDE 默认值省略参数 + 规则前置校验 |
| 重试次数下降 | **-65.7%** | 1195→410 retries，规则前置校验在调用前拦截参数错误 |
| 失败类型拆分 | 全部 5 种有对比 | timeout **71.0%**、permission **67.7%**、param_error **66.4%**、quota **62.8%**、service_unavail **60.4%**（permission 升至第二位——规模效应） |
| DAG 模式召回 | 100.0%（8/8） | 8 种预埋模式全找回 + 10 个高频子模式；**三档 degradation（500/1000/2000 seed）各档均 100%** |
| 参数模板覆盖率 | 100%（7/7） | 7 种工具 28 个参数全生成 KDE 模板；**CI 边界外总比例 0.86%（19/2200）/ per-param 最大 6.5%（arxiv 13/200）——双分母口径**；mode vs median 点估计口径约束沿用（KDE 价值在分布形状与 CI） |
| 规则准确率 | 0%（如实披露） | gsm 主口径采集点在 gsm 构建前（评测中段不产 rules）；stage 11 已接入规则蒸馏与修复建议评测（rep- 4 规则 → 4 建议，增量二） |
| 灰度上线 | **3/3 走通（310 calls）** | 三阶放量全路径实测 + 权重 40/30/30 最优（5P vs 3P）；S 终审修复后生产 canary 闭环（execute_skill 一致性哈希路由变体 + 双变体实测落 canary_invocations） |
| 权重敏感性 | 40/30/30 最优（5P vs 3P vs 3P） | 三组权重对比验证，默认权重晋升最多 |
| 简化场景 RF vs 规则 | RF **78.8%** vs 规则 **36.4%**（F1 0.670 vs 0.236） | 含拼写错误+中英混用+同义词，RF 显著优于规则；抽样依赖 _MAIN_RNG 状态，如实披露 |
| 跨语言分类 | EN **90.9%**（10/11）/ CN **66.7%**（2/3） | has_cjk 特征改善中文分类（旧 33.3% 已封存）；char_wb 跨语言仍有限 |
| 测试覆盖 | **309 tests** | 增量零 10 条生产就绪修复 + 三增量 + 去领域化 + 评测规模化 + S 终审修复（canary 闭环/异常兜底/白名单/值校验等）全部测试锁死 |
| 执行层对比（增量三） | skill_plan 成功率 **1.0**（2000/2000）、规划成本 **0ms**；llm_plan **0.991**（1982/2000）、平均规划 **1005.5ms**；**TEE 节省 token -32.5%、耗时 -25.3%**；修复闭环 **5/5** | 2000 任务 benchmark（live；llm_plan 18 失败为 4 工具长链任务 LLM 规划缺参被 fail-closed 拒绝——单任务隔离机制实测生效）；skill_plan 可复现、llm_plan 结构稳定（LLM 输出有方差——诚实口径）；**跨口径禁比较**：耗时节省低于旧口径区间下界（任务构成/并行度/LLM 时点方差），不列为"不退"证据 |
| 评测复现性 | 2000 规模二次复跑：确定性字段逐字段一致；LLM 字段结构稳定 | 种子固化（增量零建立、三增量延续；LLM 字段有运行间方差——诚实口径） |
...
└── benchmark_tasks.json   # 50 基础任务（--num-variants 扩展为 2000 确定性变体）
